...
/* Real sensor values - updated from main.c or wherever you read sensors */
int16_t accel_x = 0;
int16_t accel_y = 0;
int16_t accel_z = 0;
int16_t gyro_x = 0;
...
#define SENSOR_STRAIN		  0x03
#define SENSOR_BOTH           0x04
...
static void P2PS_Send_Strain_Data(void);
...
/* USER CODE BEGIN P2PS_APP_Init */
	UTIL_SEQ_RegTask(1 << CFG_TASK_SEND_LSM6DSO_ID, UTIL_SEQ_RFU,
			P2PS_Send_LSM6DSO_Data);
	UTIL_SEQ_RegTask(1 << CFG_TASK_SEND_STTSH22H_ID, UTIL_SEQ_RFU,
			P2PS_Send_STTSH22H_Data);
	UTIL_SEQ_RegTask(1 << CFG_TASK_SEND_STRAIN_ID, UTIL_SEQ_RFU,
			P2PS_Send_Strain_Data);
	/**
	...
	P2P_Server_App_Context.STRAIN_Active = 0;
	P2P_Server_App_Context.BothSensors_Active = 0;

	HW_TS_Create(CFG_TIM_PROC_ID_ISR,
			&(P2P_Server_App_Context.LSM6DSO_Timer_Id), hw_ts_Repeated,
			P2PS_Send_LSM6DSO_Data);
	HW_TS_Create(CFG_TIM_PROC_ID_ISR,
			&(P2P_Server_App_Context.STTSH22H_Timer_Id), hw_ts_Repeated,
			P2PS_Send_STTSH22H_Data);
	HW_TS_Create(CFG_TIM_PROC_ID_ISR, &(P2P_Server_App_Context.STRAIN_Timer_Id),
			hw_ts_Repeated, P2PS_Send_Strain_Data);
...
static void P2PS_Start_Sensor(uint8_t sensor_id) {
	...
	case SENSOR_LSM6DSO:
		if (!P2P_Server_App_Context.BothSensors_Active) {
			...
			HW_TS_Start(P2P_Server_App_Context.LSM6DSO_Timer_Id,
					interval_ticks);
	...
	case SENSOR_STTSH22H:
		if (!P2P_Server_App_Context.BothSensors_Active) {
			...
			HW_TS_Start(P2P_Server_App_Context.STTSH22H_Timer_Id,
					interval_ticks);
	...
	case SENSOR_STRAIN:
		if (!P2P_Server_App_Context.BothSensors_Active) {
			P2P_Server_App_Context.STRAIN_Active = 1;
			HW_TS_Start(P2P_Server_App_Context.STRAIN_Timer_Id, interval_ticks);
			APP_DBG_MSG("-- SENSOR: STRAIN Started (10Hz)\n\r");
		}
		break;
	...
	case SENSOR_BOTH:
		...
		HW_TS_Start(P2P_Server_App_Context.STRAIN_Timer_Id, interval_ticks);
		APP_DBG_MSG("-- SENSOR: ALL Sensors Started (10Hz)\n\r");
		break;
	}
}
...
static void P2PS_Stop_Sensor(uint8_t sensor_id) {
	...
	case SENSOR_STRAIN:
		P2P_Server_App_Context.STRAIN_Active = 0;
		HW_TS_Stop(P2P_Server_App_Context.STRAIN_Timer_Id);
		APP_DBG_MSG("-- SENSOR: STRAIN GAUGE Stopped\n\r");
		break;
...
static void P2PS_Send_LSM6DSO_Data(void) {
	...
	if (!P2P_Server_App_Context.LSM6DSO_Active
			&& !P2P_Server_App_Context.BothSensors_Active)
		return;
...
static void P2PS_Send_STTSH22H_Data(void) {
	...
	if (!P2P_Server_App_Context.STTSH22H_Active
			&& !P2P_Server_App_Context.BothSensors_Active)
		return;
...
static void P2PS_Send_Strain_Data(void) {
	...
	uint8_t payload[4];
	payload[0] = NOTIF_SENSOR_DATA;
	payload[1] = SENSOR_STRAIN;                      // SENSOR_STRAIN_GAUGE

	payload[2] = (strain_raw >> 8) & 0xFF; // MSB
	payload[3] = strain_raw & 0xFF;        // LSB
